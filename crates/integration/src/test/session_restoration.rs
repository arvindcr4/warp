...
/// The snapshot has a single window with one tab, containing:
/// * A terminal pane
/// * A settings pane (with page set to "Referrals", a section that no longer
///   exists, so restoration falls back to the default section)
    ...
    new_builder()
        ...
        .with_step(
            TestStep::new("Verify settings pane restoration")
                ...
                .add_assertion(move |app, window_id| {
                    // Verify the settings view exists; the stored "Referrals" page no
                    // longer exists, so it falls back to the default section.
